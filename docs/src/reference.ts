...
export const configDocs: readonly ConfigDoc[] = [
  ...
  {
    id: "workflow",
    path: ".github/workflows/update-dotfiles.yml",
    lang: "yaml",
    title: "update-dotfiles.yml",
    tagline: "CI that pushes config changes back into the dotfiles flake.",
    intro:
      "This workflow is the reciprocal half of the link to **naitokosuke/dotfiles**. The dotfiles flake pins this repo as an input and renders `settings.json` / `keybindings.json` through `home/vscode.nix`. When the config changes here, this job bumps the flake's locked revision — so a settings tweak reaches the real machine on the next `darwin-rebuild`.",
    primaryRef: {
    ...
    groups: [
      {
        ...
        settings: [
          {
            key: "on: push → paths: .vscode/**, keybinding.jsonc",
            summary:
              "Runs on push to `main`, but only when the actual config changed — a docs-only or README commit won't churn the flake.",
            refs: [
              {
                label: "Actions · Triggering workflows",
      ...
      {
        id: "job",
        title: "The update job",
        settings: [
          {
            key: "cachix/install-nix-action",
            summary: "Installs Nix on the runner so `nix flake update` is available.",
            refs: [
              {
                label: "cachix/install-nix-action",
                url: "https://github.com/cachix/install-nix-action",
              },
            ],
          },
          {
            key: "nix flake update vscode-settings",
            summary:
              "Checks out the *dotfiles* repo (with a PAT) and bumps just this input's locked revision in `flake.lock`.",
            refs: [
              {
                label: "Nix · nix flake update",
                url: "https://nix.dev/manual/nix/latest/command-ref/new-cli/nix3-flake-update",
              },
            ],
          },
          {
            key: "peter-evans/create-pull-request",
            summary:
              "Opens a PR against dotfiles with the refreshed lock on a self-deleting `update-vscode-settings-flake` branch.",
            refs: [
              {
                label: "peter-evans/create-pull-request",
  ...
  // ────────────────────────────────────────────────────────────────
  {
    ...
    lang: "md",
    title: "CLAUDE.md",
    tagline: "The brief Claude Code reads before touching the repo.",
    intro:
      "`CLAUDE.md` states the one fact that changes everything downstream: this is a *pure configuration* repository — no build, test, or lint step, just JSON and JSONC. It then lays out the file map, the macOS/IME gotchas, and the house rules: preserve the section comments, keep the organised structure, and weigh Japanese IME compatibility on any keyboard change.",
    primaryRef: {
      label: "Claude Code · CLAUDE.md & memory",
  ...
  // ────────────────────────────────────────────────────────────────
  {
    ...
    lang: "md",
    title: "README.md",
    tagline: "The front door — what the repo is and how it syncs.",
    intro:
      "The README says what this repo is (a config-only VS Code setup), maps the files, and documents the round-trip into the dotfiles Nix flake. It also points back at this very walkthrough.",
    groups: [],
  },
];
